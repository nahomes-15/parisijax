...
3.  **Simulate massive systems** ($N > 10^4$) on GPUs using `vmap`-accelerated MCMC.
...
where the functional $\mathcal{P}[x]$ is defined by the solution to a nonlinear partial differential equation (PDE). Let $x: [0, 1] \to [0, 1]$ be a non-decreasing function (the order parameter). We define the function $\Phi(q, y)$ on $[0, 1] \times \mathbb{R}$ as the solution to the **Parisi Backward PDE**:
